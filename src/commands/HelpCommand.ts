            ...
            options: [
                {
                    ...
                    description: i18next.t(
                        "commands:CONFIG_HELP_OPTION_DESCRIPTION",
                        { lng },
                    ),
                    type: 3,
                    required: false,
        ...
        const sections = Object.values(CommandCategory)
            .map((category) => ({
                ...
                lines: metadata
                    ...
                    .map((entry) =>
                        this.#formatCommandLine(bot.config.bot.prefix, entry),
                    ),
            }))
    ...
    async #showCommandHelp(
        ...
        for (const cmd of commands) {
            ...
            if (
                ...
                (metadata.aliases && metadata.aliases.includes(commandName))
            ) {
        ...
        if (!found) {
            await context.reply({
                embeds: [
                    embeds.textErrorMsg(
                        bot,
                        context.t("commands:MESSAGE_HELP_NOT_FOUND"),
                    ),
                ],
                allowedMentions: { repliedUser: false },
            });
